-- Migration 026: Fix units_per_display for direct master box SKUs
-- Purpose: Add pc_master.units_per_display to COALESCE chain
-- Author: Claude Code
-- Date: 2026-01-15
--
-- PROBLEM:
-- - Line 70 in Migration 025 has COALESCE missing pc_master.units_per_display
-- - Direct master box SKUs (BAKC_C02810, BAMC_C02810, KSMC_C01810) matched via pc_master
-- - But their units_per_display falls through to default of 1 instead of 5
--
-- CURRENT (WRONG):
-- COALESCE(pc_direct.units_per_display, pc_mapped.units_per_display, pc_mapped_master.units_per_display, 1)
--
-- FIXED (ADD pc_master):
-- COALESCE(pc_direct.units_per_display, pc_master.units_per_display, pc_mapped.units_per_display, pc_mapped_master.units_per_display, 1)
--
-- AFFECTED SKUs (verified):
-- | SKU          | MV Value | Expected | Orders Affected |
-- |--------------|----------|----------|-----------------|
-- | BAKC_C02810  | 1        | 5        | 43              |
-- | BAMC_C02810  | 1        | 5        | 29              |
-- | KSMC_C01810  | 1        | 5        | 6               |
...
-- ============================================
-- 2. RECREATE WITH FIXED units_per_display COALESCE
-- ============================================
...
SELECT
    ...
    -- FIX: Use actual category from product_catalog, not hardcoded 'CAJA MASTER'
    -- The is_caja_master flag already identifies these products
    COALESCE(
        pc_direct.category,
        pc_master.category,           -- FIXED: Was 'CAJA MASTER', now uses actual category
        pc_mapped.category,
        pc_mapped_master.category     -- FIXED: Was 'CAJA MASTER', now uses actual category
    ) as category,
    ...
    -- FIX: Added pc_master.units_per_display (was missing in Migration 025)
    COALESCE(pc_direct.units_per_display, pc_master.units_per_display, pc_mapped.units_per_display, pc_mapped_master.units_per_display, 1) as units_per_display,
    ...
    -- Quantity multiplier from sku_mappings (for PACK products)
...
FIXED (Migration 026): units_per_display now correctly includes pc_master lookup.
- Previously: Direct master box SKUs (BAKC_C02810) had units_per_display=1
- Now: Correctly uses pc_master.units_per_display (e.g., 5)
- Affected 78+ order lines across 3 SKUs
...
-- Verify direct master box SKUs now have correct units_per_display
SELECT
    original_sku,
    units_per_display,
    ...
    is_caja_master
FROM sales_facts_mv
WHERE original_sku IN ('BAKC_C02810', 'BAMC_C02810', 'KSMC_C01810')
GROUP BY original_sku, units_per_display, items_per_master_box, match_type, is_caja_master
LIMIT 10;

-- Expected output: units_per_display = 5 for all three SKUs
