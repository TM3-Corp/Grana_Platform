-- Migration 020: Fix sales_facts_mv to use SKU-based join with 3-path matching
-- Purpose: Fix $117M+ misclassification of CAJA MASTER products
-- Author: Claude Code
-- Date: 2025-12-17
--
-- PROBLEM:
-- - sales_facts_mv joins by product_id which is often wrong/NULL
-- - /dashboard/sales-analytics shows CAJA MASTER = $7M (should be $124M)
-- - $117M+ of CAJA MASTER sales are misclassified as CRACKERS/BARRAS/GRANOLAS
--
-- SOLUTION:
-- Use 4-path SKU matching logic:
-- 1. Direct match on product_catalog.sku → use product_name
-- 2. Match on product_catalog.sku_master → use master_box_name (CAJA MASTER)
-- 3a. Via sku_mappings → product_catalog.sku → use mapped product's data
-- 3b. Via sku_mappings → product_catalog.sku_master → use master_box_name (CAJA MASTER via ANU-/WEB)
-- 4. No match → fallback to order_items data (unmapped)
--
-- ROOT CAUSE:
-- Migration 017 line 71: LEFT JOIN products p ON oi.product_id = p.id
-- - order_items.product_id often has wrong values
-- - 2,176 items have product_id = NULL
-- - Joins to legacy 'products' table instead of 'product_catalog'
-- - Doesn't check sku_master column for CAJA MASTER products
...
-- ============================================
-- 2. RECREATE WITH 3-PATH SKU MATCHING
-- ============================================
...
SELECT
    ...
    oi.subtotal as revenue,  -- Use subtotal (excludes tax) to match Dashboard Ejecutivo
    oi.unit_price,
    oi.total,
    oi.tax_amount,
...
-- Path 3: Via sku_mappings table (for external/transformed SKUs like ANU-*, _WEB, ML IDs)
...
FIXED (Migration 020): Now uses 4-path SKU matching instead of broken product_id join:
...
Fixes $117M+ CAJA MASTER misclassification issue, including ANU- prefixed caja master SKUs.
...
-- Should show CAJA MASTER with ~$124M (not $7M)
SELECT
    category,
    match_type,
    COUNT(*) as items,
