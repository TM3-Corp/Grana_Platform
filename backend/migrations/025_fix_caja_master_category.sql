-- Migration 025: Fix CAJA MASTER category assignment in sales_facts_mv
-- Purpose: CAJA MASTER products should inherit the base product category (BARRAS, CRACKERS, etc.)
--          instead of being assigned a separate "CAJA MASTER" category
-- Author: Claude Code
-- Date: 2025-12-18
--
-- PROBLEM:
-- - CAJA MASTER SKUs (e.g., BAKC_C02810) are assigned category = 'CAJA MASTER'
-- - This excludes them from category-filtered queries like "BARRAS November"
-- - Sales Analytics shows $21.4M for BARRAS, but Audit shows $23.7M
-- - The $2.3M difference is exactly the CAJA MASTER BARRAS sales
--
-- EXAMPLE:
-- BAKC_C02810 (Caja Master Barra Keto Nuez) was:
--   - category = 'CAJA MASTER'  (WRONG - excluded from BARRAS filter)
-- Should be:
--   - category = 'BARRAS' (from product_catalog.category)
--   - is_caja_master = TRUE (flag preserved for analysis)
--
-- SOLUTION:
-- Use pc_master.category instead of hardcoding 'CAJA MASTER'
...
-- ============================================
-- 2. RECREATE WITH FIXED CATEGORY LOGIC
-- ============================================
...
-- Verify CAJA MASTER products now have correct category
SELECT
    original_sku,
    catalog_sku,
    category,
    ...
    match_type
FROM sales_facts_mv
WHERE original_sku IN ('BAKC_C02810', 'BAMC_C02810', 'BABE_C02810', 'BACM_C02810')
LIMIT 20;

-- Check BARRAS total now includes CAJA MASTER products
SELECT
    category,
    COUNT(*) as items,
    SUM(units_sold) as total_units,
    ROUND(SUM(revenue)::numeric, 0) as total_revenue,
    SUM(CASE WHEN is_caja_master THEN 1 ELSE 0 END) as caja_master_count,
    ROUND(SUM(CASE WHEN is_caja_master THEN revenue ELSE 0 END)::numeric, 0) as caja_master_revenue
FROM sales_facts_mv
WHERE source = 'relbase'
  AND order_date >= '2025-11-01'
  AND order_date <= '2025-11-30'
GROUP BY category
